...
from sklearn.linear_model import LinearRegression, Lasso, LassoCV, ElasticNetCV, LogisticRegression
...
def fit_and_evaluate_xgboost(data_train, data_test, categories = "auto", retcats = False, response_col = "income",
                             ...
                             data_transform=None, plot_importance=False):
    ...
    X_train = np.concatenate((X_train[numeric_columns_of_X].to_numpy(),
                              oh_encoder.transform(X_train[discrete_columns_of_X])),
                             axis=1)
    ...
    elif model_type.lower() in ["logistic regression"]:
        mod = LogisticRegression(**model_params)
    else:
